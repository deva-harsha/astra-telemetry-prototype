...
function ValidationEvidence() {
  return <details className="validation-evidence"><summary><span><small>Public-data research evaluation</small>Validation evidence</span><strong>View details</strong></summary><div className="validation-body"><div className="validation-stats"><span><strong>{validationEvidence.summary.backend_tests_passed}</strong> backend tests passed at Phase 1C freeze</span><span><strong>{validationEvidence.summary.public_channels_loaded}</strong> public channels loaded</span><span><strong>{validationEvidence.summary.approaches_compared}</strong> approaches compared</span><span><strong>{validationEvidence.summary.development_channels}</strong> development channels</span><span><strong>{validationEvidence.summary.holdout_channels}</strong> unseen holdout channels</span><span><strong>{validationEvidence.summary.frozen_configuration.slice(0, 12)}</strong> frozen config</span></div><div className="table-scroll"><table><thead><tr><th>Method</th><th>Micro F1</th><th>Macro F1</th><th>Event recall</th><th>Point FPR</th><th>False alerts/1,000</th></tr></thead><tbody>{validationEvidence.methods.map((method) => <tr key={method.method}><td>{method.method}</td><td>{method.micro_f1.toFixed(6)}</td><td>{method.macro_f1.toFixed(6)}</td><td>{method.event_recall.toFixed(6)}</td><td>{method.point_fpr.toFixed(6)}</td><td>{method.false_alerts_per_1000.toFixed(6)}</td></tr>)}</tbody></table></div><p className="validation-conclusion">{validationEvidence.conclusion}</p><p>{validationEvidence.disclaimer}</p><ul>{validationEvidence.limitations.map((limitation) => <li key={limitation}>{limitation}</li>)}</ul><small>Source: {validationEvidence.source}</small></div></details>
}
...
function App() {
  ...
  return <div className="app-shell"><header className="topbar"><div className="nav-brand"><strong>ASTRA</strong><span>Spacecraft Telemetry Decision Support</span></div><div className="nav-meta"><span>Simulated Telemetry</span><span>Mission {MISSION_ID}</span><span>Local Prototype</span></div></header><main>
    ...
    <section className="method-section"><div className="method-intro"><span className="kicker">Detector and source status / 04</span><h2>What is running</h2><p>The simulated telemetry demonstration and the offline public-data research evaluation use different configurations. Planned mission integration remains future work.</p></div><div className="method-columns"><div><h3>Simulated telemetry demonstration</h3><p>Fixed operating thresholds plus Isolation Forest with thermal or power trend corroboration. Three consecutive candidates confirm an event.</p></div><div><h3>Public-data research evaluation</h3><p>Offline Telemanom comparison. The frozen evaluation recommends the simpler robust threshold baseline; Isolation Forest and combined detection remain research comparisons.</p></div><div><h3>Planned mission integration</h3><p>No live spacecraft link is implemented. Integration, mission-specific limits and operational qualification remain future work.</p></div></div></section>
    <ValidationEvidence />
