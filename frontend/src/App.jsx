    ...
    {data.length === 0 ? <div className="chart-empty"><span>Telemetry plot ready</span><p>Start a simulation to reveal the 180-observation mission replay.</p></div> :
      <ResponsiveContainer width="100%" height="100%">
        <LineChart data={chartData} margin={{ top: 24, right: 18, left: 0, bottom: 6 }}>
          ...
          {faultTime && <ReferenceLine x={faultTime} stroke="#b48b50" strokeDasharray="4 4" label={{ value: 'Fault begins', position: 'insideTopRight', fill: '#c7a677', fontSize: 10 }} />}
          {detectionTime && <ReferenceLine x={detectionTime} stroke="#b66d62" strokeDasharray="2 3" label={{ value: 'Event detected', position: 'insideTopLeft', fill: '#d19489', fontSize: 10 }} />}
  ...
  return <div className="app-shell">
    ...
    <main>
      <section className="intro">
        ...
        <dl className="intro-facts"><div><dt>System status</dt><dd className={alertClass}>{activeAlert ? `${current?.severity} alert` : 'Normal'}</dd></div><div><dt>Selected scenario</dt><dd>{scenarioLabel}</dd></div><div><dt>Current observation</dt><dd>{String(visibleCount).padStart(3, '0')} <span>/ {TOTAL_POINTS}</span></dd></div></dl>
      </section>

      <section className="control-bar" aria-label="Simulation controls">
        <div className="scenario-control" role="group" aria-label="Scenario">{SCENARIOS.map((item) => <button key={item.id} type="button" className={scenario === item.id ? 'selected' : ''} aria-pressed={scenario === item.id} onClick={() => reset(item.id)}>{item.label}</button>)}</div>
      ...
      <section className="status-strip" aria-label="Current mission status">
        ...
        <div className="status-item"><span>Processing Latency</span><strong>{simulation ? simulation.metrics.processing_latency_ms.toFixed(1) : '—'}<i>ms</i></strong><small>Backend analysis time</small></div>
      </section>
      ...
      <div className="monitor-grid">
        <section className="telemetry-area" aria-labelledby="telemetry-title">
          ...
          <div className="chart-head"><div><h3>{subsystem.label}</h3><p>Signals shown relative to their normal display range. Raw values appear in the table and tooltip.</p></div><span className="chart-time">{timeLabel(current?.timestamp)}</span></div>
          ...
          <div className="table-scroll"><table><thead><tr><th>Channel</th><th>Latest value</th><th>Normal range</th><th>Trend</th><th>Detector state</th></tr></thead><tbody>{subsystem.keys.map((key) => <tr key={key}><td>{CHANNELS[key].label}</td><td className="mono-value">{reading(key, current?.[key])}</td><td className="mono-value">{CHANNELS[key].low}–{CHANNELS[key].high} {CHANNELS[key].unit}</td><td>{trend(key, current?.[key], prior?.[key])}</td><td className={activeAlert ? 'state-warning' : 'muted-cell'}>{activeAlert ? 'Persistent pattern' : 'Clear'}</td></tr>)}</tbody></table></div>
        </section>

        <aside className="event-area" aria-labelledby="event-title"><span className="kicker">Operator review / 03</span><h2 id="event-title">Event review</h2>{eventSeen ? <div className="event-content"><div className="event-state"><span className={`severity-label severity-${displayedSeverity}`}>{displayedSeverity} severity</span><span>{activeAlert ? 'Active' : 'Observed'}</span></div><h3>{event.title}</h3><dl className="event-fields"><div><dt>Affected subsystem</dt><dd>{event.affected_subsystem}</dd></div><div><dt>Event start</dt><dd>{timeLabel(event.event_start_time)}</dd></div><div><dt>Duration</dt><dd>{Math.min(event.duration, visibleCount - eventIndex)} min</dd></div><div><dt>Contributing channels</dt><dd>{event.contributing_channels.map((key) => CHANNELS[key]?.label ?? key).join(', ')}</dd></div></dl><div className="review-section"><h4>Evidence</h4><p>{event.explanation}</p></div><div className="review-section"><h4>Operator review message</h4><p>{event.operator_review_message}</p></div></div> : <div className="empty-event"><div className="empty-rule" /><h3>No active alert</h3><p>No persistent unusual telemetry pattern detected.</p><small>{simulation ? 'The replay continues to assess incoming observations.' : 'Select a scenario and start the simulation to begin review.'}</small></div>}</aside>
      </div>
